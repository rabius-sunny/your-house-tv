...
import BottomSliderDetailsDialog from './BottomSliderDetailsDialog';
...
export default function BottomSliderList({
  sliders,
  loading,
  onSliderDeleted
}: TProps) {
  ...
  const handleDelete = async (slider: Sliders[0]) => {
    if (
      !confirm(
        `Are you sure you want to delete the bottom slider "${slider.title}"?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await request.delete(`/sliders/${slider.key}?sliderKey=bottom_sliders`);
      toast.success('Bottom slider deleted successfully');
      onSliderDeleted();
    } catch (error) {
      console.error('Error deleting bottom slider:', error);
      toast.error('Failed to delete bottom slider');
    } finally {
      setDeletingKey(null);
    }
  };
    ...
    return (
      <Card>
        <CardContent className='p-12 text-center'>
          <div className='space-y-4'>
            <div>
              <h3 className='text-lg font-semibold'>No bottom sliders found</h3>
              <p className='text-muted-foreground'>
                Create your first bottom slider to get started.
              </p>
            </div>
          </div>
        </CardContent>
    ...
    <>
      <div className='space-y-4'>
        ...
        <div className='hidden md:block'>
          <Card>
            <CardHeader>
              <CardTitle>Bottom Sliders ({sliders.length})</CardTitle>
            </CardHeader>
            <CardContent>
